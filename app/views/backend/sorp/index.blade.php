...
<div id="main-content">
    ...
    <div class="page-title"> <i class="icon-custom-left"></i>
        ...
        <a href="/dashboard/sorp/create" class="btn btn-success pull-right m-20"> Nueva palabra </a>
    </div>
    <div class="row">
        <div class="col-md-12">
                            ...
                            <table cellpadding="0" cellspacing="0" border="0" class="table table-striped table-hover table-dynamic">
                                <thead>
                                    <tr>
                                        <th>ID</th>
                                        <th>Palabra</th>
                                        <th>Singular o plural</th>
                                        <th>Imagen</th>
                                        <th>Acciones</th>
                                    </tr>
                                </thead>
                                <tbody>
                                    @foreach ($sorps as $key => $sorp)      
                                    <tr>
                                        <td>{{ $sorp->id }}</td>
                                        <td>{{ $sorp->palabra }}</td>
                                        <td> {{ $sorp->sorp }} </td>
                                        <td>
                                            <img class="img-responsive imgg" src="{{ $sorp->imagen }}" alt="">
                                        </td>
                                        <td><a href="/dashboard/sorp/{{ $sorp->id }}/delete" class="btn btn-danger">Eliminar</a></td>
                                    </tr>
                                    @endforeach                                           
